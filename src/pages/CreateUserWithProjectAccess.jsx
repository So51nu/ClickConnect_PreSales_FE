...
// /* ================= SWITCH COMPONENT (YOUR ORIGINAL) ================= */
// function Switch({ checked, onChange, disabled, theme }) {
//   return (
//     <button
//       type="button"
...
//   const theme = useMemo(() => {
//     const fallback = {
//       font_family: "Inter", base_font_size: 14, background_color: "#f3f4f6",
...
//       setUsers(
//         list.filter(
//           (u) => u.role !== "SUPER_ADMIN" && u.role !== "KYC_TEAM"
//         )
//       );
...
//       const res = await axiosInstance.post(REGISTER_URL, { ...form, admin_id: adminId });
//       setCreated(res.data);
//       toast.success("User created successfully ✅");
...
//         (editingUser.project_ids || []).forEach((pid) =>
//           fd.append("project_ids", pid)
//         );
...
//   const inputStyle = { width: "100%", padding: "10px 12px", borderRadius: 12, border: "1px solid rgba(0,0,0,0.18)", outline: "none" };
//   const labelStyle = { fontSize: 12, fontWeight: 800, opacity: 0.9, marginBottom: 6 };
//   const buttonStyle = { background: theme.button_primary_bg, color: theme.button_primary_text, border: `1px solid ${theme.primary_color}`, borderRadius: 12, padding: "10px 14px", fontWeight: 800, cursor: "pointer" };
//   const smallBtnStyle = { borderRadius: 12, padding: "8px 10px", fontWeight: 900, border: `1px solid rgba(0,0,0,0.14)`, background: "#fff", cursor: "pointer" };
...
//   return (
//     <div style={{ minHeight: "100vh", background: theme.background_color, padding: "24px", fontFamily: theme.font_family }}>
//       <ToastContainer position="top-right" />
      
//       {/* 1. CREATION FORM (ORIGINAL GUI) */}
//       <div style={cardStyle}>
//         <div style={{ padding: 20 }}>
//           <form onSubmit={onSubmit}>
//             <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
...
//               <div style={{ fontSize: 14, fontWeight: 900, color: theme.primary_color }}>Project Access *</div>
//               <div style={{ display: "flex", gap: 10 }}>
...
//                   <div><div style={{ fontWeight: 900 }}>{p.name}</div><div style={{ fontSize: 12, opacity: 0.7 }}>ID: {p.id}</div></div>
...
//               <div style={{ display: "flex", alignItems: "center", gap: 10 }}><div style={{ fontWeight: 800 }}>Can View</div><Switch checked={form.can_view} onChange={v => updateData(form, setForm, "can_view", v)} theme={theme} /></div>
//               <div style={{ display: "flex", alignItems: "center", gap: 10 }}><div style={{ fontWeight: 800 }}>Can Edit</div><Switch checked={form.can_edit} onChange={v => updateData(form, setForm, "can_edit", v)} theme={theme} /></div>
//             </div>
...
//         <div style={{ padding: 20 }}>
//           <h3 style={{ marginTop: 0, color: theme.primary_color }}>Active Users</h3>
//           {usersLoading ? <div>Loading...</div> : users.map(u => (
//             <div key={u.id} style={{ display: "flex", justifyContent: "space-between", padding: "12px 0", borderBottom: "1px solid #eee", alignItems: "center" }}>
//               <div><div style={{ fontWeight: 800 }}>{u.username}</div><div style={{ fontSize: 12, opacity: 0.6 }}>{u.role} | {u.email}</div></div>
//               <div style={{ display: "flex", gap: 10 }}>
//   <button
...
//       {/* 3. EDIT MODAL (ALL FIELDS + PROJECT LIST RESTORED) */}
//       {editingUser && (
//         <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 20 }}>
//           <div style={{ background: "#fff", padding: 24, borderRadius: 16, width: "100%", maxWidth: 650, maxHeight: '90vh', overflowY: 'auto' }}>
//             <h3 style={{ marginTop: 0, color: theme.primary_color }}>Modify User: {editingUser.username}</h3>
...
//             {/* Restored Project Access in Edit Modal */}
//             <div style={{ marginTop: 16, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
...
//               <div style={{ display: "flex", alignItems: "center", gap: 10 }}><div style={{ fontWeight: 800 }}>Can View</div><Switch checked={editingUser.can_view} onChange={v => updateData(editingUser, setEditingUser, "can_view", v)} theme={theme} /></div>
//               <div style={{ display: "flex", alignItems: "center", gap: 10 }}><div style={{ fontWeight: 800 }}>Can Edit</div><Switch checked={editingUser.can_edit} onChange={v => updateData(editingUser, setEditingUser, "can_edit", v)} theme={theme} /></div>
//             </div>

//             <div style={{ display: "flex", gap: 12, marginTop: 24, justifyContent: "flex-end" }}>
//               <button onClick={() => setEditingUser(null)} style={{ padding: "10px 20px", borderRadius: 12, border: "1px solid #ddd", cursor: "pointer", background: "none", fontWeight: 700 }}>Cancel</button>

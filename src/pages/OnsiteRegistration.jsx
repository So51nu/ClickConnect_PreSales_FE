...
function extractCityAndLocalityFromPostOffices(postOffices = []) {
  ...
  for (const po of postOffices) {
    if (po.Block && po.Block !== "NA") {
      ...
      if (block.includes("east") || block.includes("west")) {
        return { city, locality: po.Block }; // ✅ Malad West, Andheri East
      }
    }
  }
  ...
  for (const po of postOffices) {
    if (po.Name && po.Name !== "NA") {
      return { city, locality: po.Name }; // ✅ Sakinaka, Orlem, Liberty Garden
    }
  }
...
export default function OnsiteRegistration() {
  ...
  // ✅ Keeping these states (no removal), but OTP is NOT required for create now
  const [quickCpOtpSending, setQuickCpOtpSending] = useState(false);
  const [quickCpOtpVerifying, setQuickCpOtpVerifying] = useState(false);
  const [quickCpOtpCode, setQuickCpOtpCode] = useState("");
  const [quickCpEmailVerified, setQuickCpEmailVerified] = useState(false);
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((res) => {
        ...
        if (data.brand) {
          if (data.brand.logo_url) setBrandLogo(data.brand.logo_url);
          if (data.brand.company_name) setCompanyName(data.brand.company_name);
        }
  ...
  const existingLeadInCurrentProject = useMemo(
    () =>
      leadsForPhone.find((lead) => String(lead.project) === String(form.project_id)) ||
      null,
    [leadsForPhone, form.project_id]
  );
  ...
  const hasLeadsInOtherProjects = useMemo(
    () => leadsForPhone.some((lead) => String(lead.project) !== String(form.project_id)),
    [leadsForPhone, form.project_id]
  );

  // 👉 Source pe for_cp true ho toh hi CP flows
  const isCpSource = useMemo(() => !!(selectedSource && selectedSource.for_cp), [selectedSource]);
  ...
  const validate = () => {
    ...
    if (!String(form.mobile_number || "").trim()) missing.push("Mobile Number");
    if (!form.unit_configuration_id) missing.push("Configuration (2/3/4 BHK)");
  ...
  const buildOnsitePayload = () => {
    return {
      ...
      residential_address: String(form.residential_address || "").trim(),
      residence_city: String(form.residence_city || "").trim(),
      residence_locality: String(form.residence_locality || "").trim(),
      residence_pincode: String(form.residence_pincode || "").trim(),

      has_channel_partner: !!isCpSource,
      channel_partner_id:
        isCpSource && form.channel_partner_id ? Number(form.channel_partner_id) : null,
  ...
  const handleScheduleSiteVisit = async () => {
    if (!hasExistingLeadInProject || !existingLeadInCurrentProject) {
      showToast("No existing lead in this project to schedule a visit.", "error");
      return;
    }
    if (!form.unit_configuration_id) {
      showToast("Please select a configuration before scheduling a visit.", "error");
      return;
    }
  ...
  const handleCopyAndCreate = async () => {
    ...
    try {
      ...
      showToast("Lead created in this project and data copied from existing lead.", "success");
      setForm(initialForm);
      setLookupResult(null);
      setCpMode(CP_MODE.REGISTERED);
    } catch (err) {
  ...
  // ---------- Quick CP: OTP send/verify (kept, but NOT required now) ----------
  const handleSendQuickCpOtp = async () => {
    const email = (quickCpForm.email || "").trim();
    if (!email) {
      showToast("Please enter CP email first.", "error");
  ...
  const filteredAssignableUsers = useMemo(() => {
    return (assignableUsers || []).filter((u) => u.role === "SALES" || u.role === "MANAGER");
  }, [assignableUsers]);

  // ✅ FIX: remove "Please verify CP email first" blocker
  const handleQuickCpCreate = async () => {
    // OTP is NOT required now, so no verification check here
  ...
  return (
    <div className="onsite-page" style={{ "--brand-logo": `url(${brandLogo})` }}>
        ...
        <div className="onsite-header">
          ...
          <div className="onsite-header-right">
            {selectedProject && (
              <>
                <h2 className="onsite-project-name">
                  {selectedProject.name || selectedProject.project_name || "Project Name"}
                </h2>
                {selectedProject.rera_number && (
                  <p className="onsite-rera-number">RERA: {selectedProject.rera_number}</p>
                )}
              </>
            )}
          </div>
        ...
        <form className="onsite-body" onSubmit={handleSubmit} noValidate>
          ...
          <div className="onsite-field">
            ...
            <select
              ...
              disabled={scopeLoading}
            >
              <option value="">{scopeLoading ? "Loading..." : "Select Project"}</option>
              {projects.map((p) => (
                <option key={p.id} value={p.id}>
                  {p.name || p.project_name || `Project #${p.id}`}
                </option>
          ...
          <div className="onsite-row-3 onsite-row-tight">
            ...
            <div className="onsite-field">
              ...
              {form.mobile_number.replace(/\D/g, "").length === 10 &&
                (checkingPhone || (lookupResult && Object.keys(lookupResult).length > 0)) && (
                  <div className="onsite-lookup-banner">
                    {checkingPhone ? (
                      <span>Checking existing records…</span>
                    ) : lookupResult?.present ? (
                ...
                existingProjectLead && (
                  <div className="onsite-helper-warning">
                    ...
                    <button
                      type="button"
                      className="onsite-link-btn"
                      onClick={() => navigate(`/leads/${existingProjectLead.id}/`)}
                    >
                      View lead
                    </button>{" "}
                    or schedule a new site visit instead of creating a new lead.
              ...
              {form.mobile_number.replace(/\D/g, "").length === 10 &&
                ...
                lookupResult.leads?.length > 0 && (
                  <div className="onsite-helper">
                    This customer already exists in another project. You can copy their data into
                    this project.
                  </div>
                )}
            </div>
          ...
          <div className="onsite-row-3 onsite-row-tight">
            ...
            <div className="onsite-field">
              ...
              <select
                className="onsite-input"
                value={form.residence_locality}
                onChange={(e) => handleChange("residence_locality", e.target.value)}
              >
                <option value="">Select Locality</option>
                {localityOptions.map((loc) => (
                  <option key={loc} value={loc}>
          ...
          <div className="onsite-row-3 onsite-row-tight">
            <div className="onsite-field">
              ...
              {mastersLoading ? (
                ...
                <select
                  className="onsite-input"
                  value={form.unit_configuration_id}
                  onChange={(e) => handleChange("unit_configuration_id", e.target.value)}
                >
                  <option value="">Select Configuration</option>
                  {unitConfigs.map((cfg) => (
                    <option key={cfg.id} value={cfg.id}>
                      {cfg.name || cfg.label || cfg.configuration || `Config #${cfg.id}`}
                    </option>
                  ))}
                </select>
              )}
            ...
            <div className="onsite-field">
              ...
              {mastersLoading ? (
                ...
                <div className="onsite-helper">
                  {selectedProject ? "No sources configured." : "Select a project to see sources."}
                </div>
              ) : (
                <select
                  className="onsite-input"
          ...
          {isCpSource ? (
            <div className="onsite-row-3 onsite-row-tight">
              <div className="onsite-field">
                ...
                {purposes.length === 0 ? (
                  <div className="onsite-helper">
                    {selectedProject ? "No purposes configured." : "Select a project to see purposes."}
                  </div>
                ) : (
                  <select
                    className="onsite-input"
              ...
              <div className="onsite-field">
                ...
                <select
                  ...
                  onChange={(e) => {
                    const nextMode = e.target.value;
                    setCpMode(nextMode);
                    setForm((prev) => ({ ...prev, channel_partner_id: "" }));
                  }}
                >
                  <option value={CP_MODE.REGISTERED}>Registered</option>
                  <option value={CP_MODE.UNREGISTERED}>Unregistered</option>
              ...
              <div className="onsite-field">
                ...
                {cpMode === CP_MODE.REGISTERED ? (
                  <select
                    className="onsite-input"
                    value={form.channel_partner_id}
                    onChange={(e) => handleChange("channel_partner_id", e.target.value)}
                    disabled={cpLoading}
                  >
                    <option value="">{cpLoading ? "Loading..." : "Select Channel Partner"}</option>
                    {channelPartners.map((cp) => {
                      const fullName =
                        cp.full_name ||
                        cp.name ||
                        [cp.first_name, cp.last_name].filter(Boolean).join(" ");
                      const label = fullName || cp.company_name || cp.email || `CP #${cp.id}`;
                      return (
                        <option key={cp.id} value={cp.id}>
                          {label}
                        </option>
      ...
      {/* ✅ Quick CP Create Modal (OTP UI removed, direct create) */}
      {showQuickCpModal && (
        <div className="modal-overlay">
          <div className="modal" style={{ maxWidth: "600px" }}>
            <h3 className="modal-title">Create New Channel Partner</h3>
            ...
            <div style={{ marginBottom: "15px" }}>
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  // keeping resets (no removal)
                  setQuickCpEmailVerified(false);
                  setQuickCpOtpCode("");
                }}
              />
              <div style={{ fontSize: 12, color: "#6b7280", marginTop: 6 }}>
                OTP verification removed — CP will be created directly.
              </div>
            </div>
      ...
      {showSuccessModal && (
        <div className="modal-overlay">
          ...
          >
            ...
            <p style={{ fontSize: 14, color: "#4b5563", marginBottom: 16 }}>
              Thank you for taking the time to fill out our registration form and for trusting us
              with your personal details. We truly appreciate it.
            </p>

            <p style={{ fontSize: 14, color: "#6b7280", marginBottom: 18 }}>
              Please rest assured that your information is safe with us. We respect your privacy,
              and you will never receive unnecessary calls or messages from our side — whether or
              not you decide to purchase a property with us.
            </p>
            ...
            <div className="modal-actions" style={{ justifyContent: "center" }}>
              <button
                ...
                onClick={() => {
                  setShowSuccessModal(false);
                  if (successLeadId) navigate(`/leads/${successLeadId}/`);
                }}
              >
                DONE
              </button>
                  ...
                  {lead.last_update ? (
                    ...
                    >
                      ...
                      <div style={{ marginTop: 2 }}>
                        <strong>On:</strong>{" "}
                        {lead.last_update.event_date
                          ? lead.last_update.event_date.slice(0, 16).replace("T", " ")
                          : "-"}
                        {"  "} | <strong>By:</strong> {lead.last_update.created_by || "-"}
                      </div>
                    </div>
                  ) : (
                    <div style={{ fontSize: 13, marginTop: 4 }}>No updates recorded yet.</div>
                  )}
                </div>
              ))
            )}
